...
class ContentBot {
   ...
   * @param {string} options.researchMode - 'basic' or 'enhanced' (default: 'enhanced')
   * @param {number} options.bingCount - Number of Bing news articles to fetch (default: 5)
   * @param {number} options.tavilyCount - Number of Tavily search results to fetch (default: 5)
  ...
  async generateTopics(category, options = {}) {
    ...
    const topicsGenerator = require('../bin/generate-topics');
...
const createBlogFunctions = require('../bin/create-blog');
const generateTopicsFunctions = require('../bin/generate-topics');
